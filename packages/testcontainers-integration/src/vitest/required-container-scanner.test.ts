...
test(
  'given the legacy single-container form, when requirements are discovered, then it remains supported',
  async () => {
    const root = await mkdtemp(join(tmpdir(), 'legacy-container-requirements-'));
    try {
      await writeFile(
        join(root, 'candidate.container.integration.test.ts'),
        '@RequiredContainer(Container.PostgreSql) class CandidateIntegrationTest {}',
      );
